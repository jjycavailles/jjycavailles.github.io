...
to setup-infected
  ask n-of initialement-infecte turtles [
   set color red
   set infected? true
  ]
end
...
to infect-susceptibles ;; S -> I
  ask turtles [
    ...
    if (random-float 1 <  1 - (((1 - transmissibilite) ^ infected-neighbors)) and not immune?)
    [set infected? true]
  ]
end
...
to move
  ask turtles [
    right 90 - random 180 ;;get a new random heading
    forward 1
  ]
end
...
to recover-infected ;;I -> R
  ask turtles with [infected?]
  [
    if random-float 1 < taux-de-guerison    [
      set infected? false
      set immune? true
      set color gray
    ]
...
"" ""
PENS
"Infectieux" 1.0 0 -2674135 true "" "plot (count turtles with [infected?]) / num-turtles"
"susceptible" 1.0 0 -10899396 true "" "plot (count turtles with [not infected? and not immune?]) / num-turtles"
"guéris" 1.0 0 -7500403 true "" "plot (count turtles with [immune?]) / num-turtles"
